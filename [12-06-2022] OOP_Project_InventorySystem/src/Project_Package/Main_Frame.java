...
public class Main_Frame extends javax.swing.JFrame {
    ...
    public Main_Frame() {
        ...
        for(int a = 0; a < profitHistory.size(); a++) {
            System.out.println(formatCurrency(roundDecimals(profitHistory.get(a))));
        }
        ...
        Main_purchaseInformationLayout.setVerticalGroup(
            ...
            .addGroup(Main_purchaseInformationLayout.createSequentialGroup()
                ...
                .addGroup(Main_purchaseInformationLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    ...
                    .addComponent(Main_inputPanel, javax.swing.GroupLayout.DEFAULT_SIZE, 595, Short.MAX_VALUE))
    ...
    private void Main_pay_BtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Main_pay_BtnActionPerformed
        ...
        if(cash < amountToPay){
            ...
            */
            ...
            /* - Checking Purposes
            System.out.println("Vatable: " + totalVatableSalesResult);
            System.out.println("Vatable Excempt: " + totalVatExcemptSalesResult);
            System.out.println("VAT: " + totalVATSalesResult);
            */
            ...
            String receiptHeader = "RECEIPT#" + referenceGenerated + "\n" +
                                   ...
                                   "CITICITY, ROSTAM (NEW ALBAY NIEVO) \n" +
                                   "-----------------------------------------\n\n";
            ...
            for(int i = 0; i < Main_cartViewer_Table.getRowCount(); i++){
                String receiptContents = String.format("%-" + idLength + "s  ", mainCart.getValueAt(i, 0).toString()) +
                                         ...
                                         String.format("%-" + priceLength + "s  ", mainCart.getValueAt(i, 5).toString()) +
                                         String.format("%2s  \n\n", mainCart.getValueAt(i, 6).toString());
                Main_receipt_TxtPane.append(receiptContents);
            }
            
            String receiptFooter = "-----------------------------------------\n" +
                                   "VATABLE : " + totalVatableSalesResult + "\n" +
                                   "VAT EXEMPT : " + totalVatExcemptSalesResult + "\n" +
                                   "VAT : " + totalVATSalesResult + "\n\n" +
                                   "TOTAL  : " + cartTotal() + "\n" +
                                   "CASH   : " + cash + "\n" +
                                   "CHANGE : " + changeAmount + "\n\n" +
                                   "Thank You For Shopping At Citi Megamalls!";
            Main_receipt_TxtPane.append(String.format("%s", receiptFooter));

            /* PRINTING in PDF ATTEMPT
                MessageFormat a = new MessageFormat("-");
                MessageFormat b = new MessageFormat("-");
            
                try
                {
                    Main_receipt_TxtPane.print(a, b);
                }
                catch (Exception e)
                {
    ...
    private void Sales_receiptList_TableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_Sales_receiptList_TableMouseClicked
        ...
        String referenceNumberSelected = referenceNumbers.get(selectedReferenceNumber);
    ...
    public static double roundDecimals(double number) {
        int accuracy = 2;
        double estimate = number * Math.pow(10,accuracy);
		
        return Math.round(Math.round(estimate) / Math.pow(10,accuracy));
    }
